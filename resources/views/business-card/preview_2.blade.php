...
<body style="background:#6B6B6B">
    ...
    @foreach($businessCard as $b)

    <div class="card mb-3">
        <img class="card-img-top" src="{{ asset($b->design) }}" alt="Card image cap" width="150" height="300">
        <div class="card-body">
          <h5 class="card-title">Business Card Preview</h5>
          <p class="card-text"><p><b>Name: {{ $b->name }}</b></p>
          <p class="card-text"><p><b>Role: {{ $b->role }}</b></p>
          <p class="card-text"><p><b> Company Name: {{ $b->company_name }}</b></p>

          <p class="card-text"><small class="text-muted">created_at: {{ $b->created_at }}</small></p>
        </div>
      </div>
      @endforeach
